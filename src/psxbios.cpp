...
#define buopen(mcd) { \
 ...
 \
			...
			/*printf("openC %s\n", ptr);*/ \
			v0 = 1 + mcd; \
			SaveMcd((mcd==1?MCD1_FILE:MCD2_FILE), Mcd##mcd##Data, 128 * i, 128); \
			break; \
		} \
	} \
}
...
#define buwrite(mcd) { \
	...
	SaveMcd((mcd==1?MCD1_FILE:MCD2_FILE), Mcd##mcd##Data, offset, a2); \
	if (FDesc[1 + mcd].mode & 0x8000) v0 = 0; \
	else v0 = a2; \
	DeliverEvent(0x11, 0x2); /* 0xf0000011, 0x0004 */ \
	DeliverEvent(0x81, 0x2); /* 0xf4000001, 0x0004 */ \
...
#define burename(mcd) { \
	for (i=1; i<16; i++) { \
		...
		for (j=0; j<127; j++) cxor^= ptr[j]; \
		ptr[127] = cxor; \
		SaveMcd((mcd==1?MCD1_FILE:MCD2_FILE), Mcd##mcd##Data, 128 * i + 0xa, 0x76); \
		v0 = 1; \
		break; \
	} \
}
...
#define budelete(mcd) { \
	for (i=1; i<16; i++) { \
		...
		if (strcmp(Ra0+5, ptr+0xa)) continue; \
		*ptr = (*ptr & 0xf) | 0xA0; \
		SaveMcd((mcd==1?MCD1_FILE:MCD2_FILE), Mcd##mcd##Data, 128 * i, 1); \
		/*printf("delete %s\n", ptr+0xa);*/ \
		v0 = 1; \
		break; \
	} \
...
void psxBios__card_write(void) { // 0x4e
	...
	if (port == 0) {
		...
		SaveMcd(MCD1_FILE, Mcd1Data, a1 * 128, 128);
	} else {
		memcpy(Mcd2Data + a1 * 128, Ra2, 128);
		SaveMcd(MCD2_FILE, Mcd2Data, a1 * 128, 128);
	}
